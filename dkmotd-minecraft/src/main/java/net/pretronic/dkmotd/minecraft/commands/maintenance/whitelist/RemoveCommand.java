...
public class RemoveCommand extends BasicCommand {
    ...
    public RemoveCommand(ObjectOwner owner, DKMotd dkMotd) {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if(!maintenance.isWhitelisted(target.getUniqueId())) {
            sender.sendMessage(Messages.ERROR_MAINTENANCE_WHITELIST_PLAYER_NOT, VariableSet.create().addDescribed("player", target));
            return;
        }
        if(maintenance.removeWhitelist(target.getUniqueId())) {
            sender.sendMessage(Messages.COMMAND_MAINTENANCE_WHITELIST_REMOVE, VariableSet.create().addDescribed("player", target));
        }
    }
}
